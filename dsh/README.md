# web-tool-dsh — DeepSeek Harness 适配插件

把 [web-tool](../README.md)（零依赖 AI 网页抓取器）注册为 DSH agent 的 **`web_fetch`** / **`web_search_bing`** 两个工具。
...
```sh
# 1. 安装 npm 包（含 web-tool 依赖）
npm i -g web-tool-dsh        # 或装到 DSH profile 的 node_modules

# 2. 把 bundle 加入 profile
#    编辑 ~/.dsh/profiles/<profile>/package.json：
#    "dsh": { "profile": { "bundles": [..., "web-tool-dsh"] } }
```
...
> 用 `cordis_define`（idPrefix: `webt`，code.host 为 `examples/dsh-plugin.mjs` 的函数体，把 `WEB_TOOL` 改为 `require('web-tool/package.json')` 解析出的 cli.js 路径），然后 `cordis_run`。工具立即在当前会话可用；重启后需重装（动态插件不持久）。
...
- 执行走 `ctx.subprocess` spawn `node web-tool cli.js --json`（`exec.signal` 绑定进程树终止）
